...
print("=" * 70)
print("PROMPT 6: NẠP KNOWLEDGE GRAPH MINI VÀO NEO4J (LAB_SESSION = 'buoi_15')")
print("=" * 70)
...
with driver.session() as session:
    # 1. Dọn dẹp an toàn chỉ dữ liệu của buoi_15
    session.run("MATCH (n {lab_session: 'buoi_15'}) DETACH DELETE n")
    ...
    for _, r in df_corpus.iterrows():
        ...
        session.run("""
            ...
            MERGE (v)-[:CONTAINS {lab_session: 'buoi_15'}]->(d)
        ...
        if prev_doc_id == d_id and prev_chunk_id:
            session.run("""
                ...
                MERGE (d1)-[:NEXT {lab_session: 'buoi_15'}]->(d2)
    ...
    for _, r in df_rel.iterrows():
        ...
        session.run(f"""
            ...
            MERGE (v1)-[:{rel_type} {{lab_session: 'buoi_15'}}]->(v2)
        """, doc1=str(r["doc_id"]), doc2=str(r["other_doc_id"]))

    res_vb = session.run("MATCH (n:VanBan {lab_session: 'buoi_15'}) RETURN count(n) as count").single()["count"]
    res_dk = session.run("MATCH (n:DieuKhoan {lab_session: 'buoi_15'}) RETURN count(n) as count").single()["count"]
    res_rel = session.run("MATCH ()-[r {lab_session: 'buoi_15'}]->() RETURN count(r) as count").single()["count"]
